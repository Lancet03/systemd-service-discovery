...
```powershell
podman run -d --name worker_1 --privileged --systemd=always --network host -e DISCOVERY_URL=http://10.89.0.1:8080 -e SERVICE_ID=worker_1 -e SERVICE_DESCRIPTION='Worker 1 inside systemd container' worker-systemd:latest
```
